...
export class PropertyService {
	...
	static async createProperty(
		...
		try {
			...
			logger.info(`New property created: ${property.title} by ${owner.email}`);
	...
	// Get property by ID
	static async getPropertyById(propertyId: string): Promise<IProperty | null> {
		try {
	...
	static async getAllProperties(queryParams: IQueryParams): Promise<{
		...
		try {
			...
			if (filter.city) {
				query["location.city"] = { $regex: filter.city, $options: "i" };
			}
			if (filter.state) {
				query["location.state"] = { $regex: filter.state, $options: "i" };
			}
